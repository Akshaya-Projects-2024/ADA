...
export const ActivityList = (props) => {
  ...
  const activityList = props?.route?.params?.acivityList;

  // useEffect(() => {
  //   contextValue?.setLoader(true);
  //   const activityListData = props?.route?.params?.activityListData;
  //   const initialList =
  //     Array.isArray(activityListData) && activityListData.length > 0
  //       ? activityListData
  //       : Object.values(DefaultActivityList);
  //   setActivityDataList(initialList);
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        {Boolean(activityDataList.length) && (
          <View style={{ marginVertical: 20 }}>
            {getCurrentRoute() == "activityTrackerList" && (
              <Button
                title="Add Activity"
                onPress={() => {
                  handleActivityType("Custom");
                }}
                onlyBorder
              />
            )}
